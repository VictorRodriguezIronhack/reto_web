...
router.post('/registro', (req, res) => {
    ...
    if (password.length < 5) {
        res.status(400).json({ message: 'Contraseña demasiado corta.Debe contener mínimo 5 caracteres' })
        return
    }
...
router.get('/sesioniniciada', (req, res) => req.isAuthenticated() ? res.status(200).json(req.user) : res.status(403).json({ message: 'Desautorizado' }))
